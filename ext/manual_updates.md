...
| File                                     | Reason                                                                                                                                                                                                                |
|------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `opencv2/dnn/version-rb.cpp`             | Add `#define CV_DNN_DONT_ADD_INLINE_NS // Manual` on line 1 before includes. The `-DCV_DNN_DONT_ADD_INLINE_NS` in rice-bindings.yaml only affects parse time (libclang); this `#define` is still needed at compile time to suppress the inline namespace in `version.hpp` |
