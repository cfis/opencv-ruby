...
void Init_Aruco()
{
  ...
  rb_mCvAruco.define_module_function("estimate_pose_single_markers", &cv::aruco::estimatePoseSingleMarkers,
    Arg("corners"), Arg("marker_length"), Arg("camera_matrix"), Arg("dist_coeffs"), Arg("rvecs"), Arg("tvecs"), Arg("obj_points") = static_cast<cv::OutputArray>(cv::noArray()), Arg("estimate_parameters") = static_cast<const cv::Ptr<cv::aruco::EstimateParameters>&>(makePtr<cv::aruco::EstimateParameters>()));
