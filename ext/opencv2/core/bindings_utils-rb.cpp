...
void Init_Core_BindingsUtils()
{
  ...
  rb_mCvUtils.define_module_function<cv::String(*)(int, const cv::Point &)>("test_overload_resolution", static_cast<cv::String(*)(int, const cv::Point &)>(&cv::utils::testOverloadResolution),
    Arg("value"), Arg("point") = static_cast<const cv::Point &>(cv::Point(42, 24)));

  rb_mCvUtils.define_module_function<cv::String(*)(const cv::Rect &)>("test_overload_resolution", static_cast<cv::String(*)(const cv::Rect &)>(&cv::utils::testOverloadResolution),
    Arg("rect"));
