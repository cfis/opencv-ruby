...
{
  auto klass = Rice::define_class_under<cv::Matx<_Tp, m, n>>(parent, name).
  ...
  // Manual - Dimension-specific constructors
  if constexpr (m * n == 1)
  {
  ...
  // Manual - Float/double-only methods
  ...
  // Manual - Vector-only single-index operator
...
{
  auto klass = Rice::define_class_under<cv::Vec<_Tp, cn>, cv::Matx<_Tp, cn, 1>>(parent, name). // Manual - inherit from Matx
  ...
  // Manual - Channel-specific constructors
  if constexpr (cn == 1)
  {
  ...
  // Manual - Complex conjugate method (only for 2 or 4 channel float/double)
  ...
  // Manual - Cross product method (only for 3-channel)
  if constexpr (cn == 3)
  {
