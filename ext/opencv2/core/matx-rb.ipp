...
inline Rice::Data_Type<cv::Matx<_Tp, m, n>> Matx_instantiate(Rice::Module parent, const char* name)
{
  ...
  // Manual - Dimension-specific constructors wrapped in if constexpr
  if constexpr (m * n >= 1)
  {
...
inline Rice::Data_Type<cv::Vec<_Tp, cn>> Vec_instantiate(Rice::Module parent, const char* name)
{
  ...
  // Manual - Channel-specific constructors wrapped in if constexpr
  if constexpr (cn >= 1)
  {
  ...
  // Manual - Complex conjugate method (only for Vec2/Vec4 with float/double)
  ...
  // Manual - Cross product method (only for Vec3)
  if constexpr (cn == 3)
  {
