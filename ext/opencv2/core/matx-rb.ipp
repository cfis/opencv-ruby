...
inline Rice::Data_Type<cv::Matx<_Tp, m, n>> Matx_instantiate(Rice::Module parent, const char* name)
{
  ...
  // Manual - Dimension-specific constructors wrapped in if constexpr guards
  if constexpr (m * n >= 1)
  {
  ...
  // Manual - inv/solve only available for float/double types
  ...
  // Manual - Single-index operator() only valid for vectors (1D)
...
inline Rice::Data_Type<cv::Vec<_Tp, cn>> Vec_instantiate(Rice::Module parent, const char* name)
{
  ...
  // Manual - Channel-specific constructors wrapped in if constexpr guards
  if constexpr (cn >= 1)
  {
  ...
  // Manual - conj only available for 2/4 channel float/double (complex number operations)
  ...
  // Manual - cross product only for 3D vectors
  if constexpr (cn == 3)
  {
