...
inline Rice::Data_Type<cv::Matx<_Tp, m, n>> Matx_instantiate(Rice::Module parent, const char* name)
{
  ...
  // Manual - single-index operator() only available for vectors (m == 1 || n == 1)
...
inline Rice::Data_Type<cv::Vec<_Tp, cn>> Vec_instantiate(Rice::Module parent, const char* name)
{
  ...
  // Manual - conj only available for 2/4 channel float/double (complex types)
  ...
  // Manual - cross only available for 3-channel vectors
  if constexpr (cn == 3)
  {
