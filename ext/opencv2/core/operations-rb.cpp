...
void Init_Core_Operations()
{
  ...
  rb_mCv.define_module_function<cv::Ptr<cv::Formatted>(*)(cv::InputArray, cv::Formatter::FormatType)>("format", static_cast<cv::Ptr<cv::Formatted>(*)(cv::InputArray, cv::Formatter::FormatType)>(&cv::format),
    Arg("mtx"), Arg("fmt"));

  rb_mCv.define_module_function<int(*)(cv::Ptr<cv::Formatted>, FILE *)>("print", static_cast<int(*)(cv::Ptr<cv::Formatted>, FILE *)>(&cv::print),
    Arg("fmtd"), Arg("stream") = static_cast<FILE *>(stdout));

  rb_mCv.define_module_function<int(*)(const cv::Mat &, FILE *)>("print", static_cast<int(*)(const cv::Mat &, FILE *)>(&cv::print),
    Arg("mtx"), Arg("stream") = static_cast<FILE *>(stdout));

  rb_mCv.define_module_function<int(*)(const cv::UMat &, FILE *)>("print", static_cast<int(*)(const cv::UMat &, FILE *)>(&cv::print),
    Arg("mtx"), Arg("stream") = static_cast<FILE *>(stdout));
