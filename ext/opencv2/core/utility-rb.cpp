...
inline void AutoBuffer_builder(Data_Type_T& klass)
{
  klass.define_constructor(Constructor<cv::AutoBuffer<_Tp, fixed_size>>()).
    ...
    define_constructor(Constructor<cv::AutoBuffer<_Tp, fixed_size>, const cv::AutoBuffer<_Tp, fixed_size>&>(),
      Arg("buf")).
    define_method("assign", &cv::AutoBuffer<_Tp, fixed_size>::operator=,
      Arg("buf")).
    define_method("allocate", &cv::AutoBuffer<_Tp, fixed_size>::allocate,
      Arg("_size")).
    define_method("deallocate", &cv::AutoBuffer<_Tp, fixed_size>::deallocate).
    define_method("resize", &cv::AutoBuffer<_Tp, fixed_size>::resize,
      Arg("_size")).
    define_method("size", &cv::AutoBuffer<_Tp, fixed_size>::size).
...
inline void Node_builder(Data_Type_T& klass)
{
  klass.define_constructor(Constructor<cv::Node<OBJECT>>()).
    ...
    template define_method<int(cv::Node<OBJECT>::*)(cv::Node<OBJECT>*) const>("find_child", &cv::Node<OBJECT>::findChild,
      Arg("p_node")).
    define_method("add_child", &cv::Node<OBJECT>::addChild,
      Arg("p_node")).
    define_method("remove_childs", &cv::Node<OBJECT>::removeChilds).
    define_method("get_depth", &cv::Node<OBJECT>::getDepth).
...
void Init_Core_Utility()
{
  ...
  Rice::Data_Type<cv::TickMeter> rb_cCvTickMeter = define_class_under<cv::TickMeter>(rb_mCv, "TickMeter").
    ...
    define_method("get_last_time_ticks", &cv::TickMeter::getLastTimeTicks).
    define_method("get_last_time_micro", &cv::TickMeter::getLastTimeMicro).
    define_method("get_last_time_milli", &cv::TickMeter::getLastTimeMilli).
    define_method("get_last_time_sec", &cv::TickMeter::getLastTimeSec).
  ...
  rb_mCv.define_module_function("get_default_algorithm_hint", &cv::getDefaultAlgorithmHint);
#endif
