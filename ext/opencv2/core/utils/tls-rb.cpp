...
inline void TLSData_builder(Data_Type_T& klass)
{
  klass.define_constructor(Constructor<cv::TLSData<T>>()).
    define_method("get", &cv::TLSData<T>::get).
    define_method("get_ref", &cv::TLSData<T>::getRef).
    define_method("cleanup", &cv::TLSData<T>::cleanup);
};
...
inline void TLSDataAccumulator_builder(Data_Type_T& klass)
{
  klass.define_constructor(Constructor<cv::TLSDataAccumulator<T>>()).
    define_method("gather", &cv::TLSDataAccumulator<T>::gather,
      Arg("data")).
    define_method("detach_data", &cv::TLSDataAccumulator<T>::detachData).
    define_method("cleanup_detached_data", &cv::TLSDataAccumulator<T>::cleanupDetachedData).
    define_method("cleanup", &cv::TLSDataAccumulator<T>::cleanup).
    define_method("release", &cv::TLSDataAccumulator<T>::release);
};
void Init_Tls()
{
