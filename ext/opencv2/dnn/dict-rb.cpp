...
void Init_Dnn_Dict()
{
  ...
  Rice::Data_Type<cv::dnn::DictValue> rb_cCvDnnDictValue = define_class_under<cv::dnn::DictValue>(rb_mCvDnn, "DictValue").
    ...
    define_constructor(Constructor<cv::dnn::DictValue, const char*>(),
      Arg("s")).
    define_method("size", &cv::dnn::DictValue::size).
    define_method("int?", &cv::dnn::DictValue::isInt).
    define_method("string?", &cv::dnn::DictValue::isString).
    define_method("real?", &cv::dnn::DictValue::isReal).
    define_method("get_int_value", &cv::dnn::DictValue::getIntValue,
      Arg("idx") = static_cast<int>(-1)).
    define_method("get_real_value", &cv::dnn::DictValue::getRealValue,
      Arg("idx") = static_cast<int>(-1)).
    define_method("get_string_value", &cv::dnn::DictValue::getStringValue,
      Arg("idx") = static_cast<int>(-1)).
