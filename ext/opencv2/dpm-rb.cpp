...
void Init_Dpm()
{
  ...
  Rice::Data_Type<cv::dpm::DPMDetector> rb_cCvDpmDPMDetector = define_class_under<cv::dpm::DPMDetector>(rb_mCvDpm, "DPMDetector").
    define_method("empty?", &cv::dpm::DPMDetector::isEmpty).
    define_method("detect", &cv::dpm::DPMDetector::detect,
      Arg("image"), Arg("objects")).
    define_method("get_class_names", &cv::dpm::DPMDetector::getClassNames).
    define_method("get_class_count", &cv::dpm::DPMDetector::getClassCount).
    define_singleton_function("create", &cv::dpm::DPMDetector::create,
