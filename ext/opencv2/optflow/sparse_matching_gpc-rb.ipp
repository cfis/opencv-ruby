...
inline Rice::Data_Type<cv::optflow::GPCForest<T>> GPCForest_instantiate(Rice::Module parent, const char* name)
{
  return Rice::define_class_under<cv::optflow::GPCForest<T>>(parent, name).
    ...
    template define_method<void(cv::optflow::GPCForest<T>::*)(const std::vector<cv::String>&, const std::vector<cv::String>&, const std::vector<cv::String>&, const cv::optflow::GPCTrainingParams)>("train", &cv::optflow::GPCForest<T>::train,
