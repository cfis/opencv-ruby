...
class SparseMat_Test < OpenCVTestCase
  ...
  def test_initialize_with_sizes
    ...
    mat = Cv::SparseMat1f.new(buffer.size, buffer.data)
  ...
  def test_getter
    sizes = [2, 2]
    buffer = Rice::Buffer≺int≻.new(sizes)
    sparse = Cv::SparseMat1b.new(buffer.size, buffer.data)
    assert_equal(0, sparse[0, 0])

    sparse = Cv::SparseMat2b.new(buffer.size, buffer.data)
  ...
  def test_test_update
    sizes = [2, 2]
    buffer = Rice::Buffer≺int≻.new(sizes)
    mat = Cv::SparseMat1b.new(buffer.size, buffer.data)
    assert_equal([], mat.each.to_a)
    mat[2, 3] = 7
    mat[3, 4] = 8
    assert_equal([8, 7], mat.each.to_a)

    mat = Cv::SparseMat2b.new(buffer.size, buffer.data)
    assert_equal([], mat.each.to_a)
