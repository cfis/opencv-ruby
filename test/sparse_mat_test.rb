...
class SparseMatTest < OpenCVTestCase
	...
	def test_initialize_with_sizes
		...
		mat = Cv::SparseMat.new(buffer.size, buffer.data, CV_32FC1)
		...
		assert_kind_of(Rice::Pointer≺int≻, mat.size)
		buffer = Rice::Buffer≺int≻.new(mat.size, sizes.count)
		assert_equal(10, buffer[0])
		assert_equal(20, buffer[1])
	end
